...
def changePassword(session, oldpassword, password, user_id, access_token, proxy):
    ...
    b = session.put("https://www.nike.com/profile/services/users/" + user_id + "/password", json=changedata,
...
def accountInfos(session, proxy):
    ...
    try:
        ...
        log(
            "Canno't scrape verified phone neither country, it probably means that this account is not verified, please check the error below")
        print(e)
    try:
        ...
        log(
            "Canno't scrape nuId or upmId, it probably means that the login didn't work, please check the error below and retry")
        exit(0)
...
def retrievePayMethods(session, proxy, profile):
    ...
    a = session.post(
        'https://api.nike.com/commerce/storedpayments/consumer/storedpayments?currency=EUR&includeBalance=true',
...
def co3(session, proxy, sku_uuid, profile, shippingUUID, deviceId):
    ...
    a = session.put('https://api.nike.com/buy/checkout_previews/v2/' + checkoutUUID, verify=False, headers=headers,
                    json=payload)
...
def setPayment(session, proxy, profile):
    ...
    if (payments):
        for i in payments:
            ...
            # usual headers
            headers = {
                "authority": "api.nike.com", "method": "DELETE",
                "path": "/commerce/storedpayments/consumer/storedpayments/" + paymentID,
                "scheme": "https", "accept": "*/*", "accept-encoding": "gzip;q=1.0, compress;q=0.5",
                "accept-language": "fr-FR;q=1.0, en-US;q=0.9, zh-Hans-FR;q=0.8",
                "Authorization": "Bearer " + access_token, "content-type": "application/json",
                "user-agent": "SNEAKRS/1.1.3 (iPhone; iOS 11.2.2; Scale/3.00)",
                "x-nike-caller-id": "nike:sneakrs:ios:1.1",
            }
            r = session.delete("https://api.nike.com/commerce/storedpayments/consumer/storedpayments/" + paymentID,
    ...
    headers = {
        "authority": "api.nike.com", "method": "POST",
        "path": "/paymentcc/creditcardsubmit/creditcardsubmit/" + newPaymentID + "/store",
        "scheme": "https", "accept": "*/*", "accept-encoding": "gzip;q=1.0, compress;q=0.5",
        "accept-language": "fr-FR;q=1.0, en-US;q=0.9, zh-Hans-FR;q=0.8",
        "Authorization": "Bearer " + access_token, "content-type": "application/json",
        "user-agent": "SNEAKRS/1.1.3 (iPhone; iOS 11.2.2; Scale/3.00)",
    ...
    r = session.post("https://api.nike.com/paymentcc/creditcardsubmit/creditcardsubmit/" + newPaymentID + "/store",
                     verify=False, headers=headers, json=payload)
    print(r)
    headers = {
        "authority": "api.nike.com", "method": "GET",
        "path": "/paymentcc/creditcardsubmit/creditcardsubmit/" + newPaymentID + "/isValid",
        "scheme": "https", "accept": "*/*", "accept-encoding": "gzip;q=1.0, compress;q=0.5",
        "accept-language": "fr-FR;q=1.0, en-US;q=0.9, zh-Hans-FR;q=0.8",
        "Authorization": "Bearer " + access_token, "content-type": "application/json",
        "user-agent": "SNEAKRS/1.1.3 (iPhone; iOS 11.2.2; Scale/3.00)",
    ...
    r2 = session.get("https://api.nike.com/paymentcc/creditcardsubmit/creditcardsubmit/" + newPaymentID + "/isValid",
                     verify=False, headers=headers)
    print(r2)
    ...
    headers = {
        ...
        "accept-language": "fr-FR;q=1.0, en-US;q=0.9, zh-Hans-FR;q=0.8",
        "Authorization": "Bearer " + access_token, "content-type": "application/json",
        "user-agent": "SNEAKRS/1.2.2 (iPhone; iOS 11.2.2; Scale/3.00)",
    ...
    r3 = session.post("https://api.nike.com/commerce/storedpayments/consumer/savepayment", verify=False,
...
login(s, "1712071591@qq.com", "Fdh123456789", proxy)
...
entryUUID = entry1(s, proxy, profile, deviceId, checkoutUUID, paymentUUID, skuUUID, PCS,
                   launchID)  # sku uuid launchid pricecheksum
